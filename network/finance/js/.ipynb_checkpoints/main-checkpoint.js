...
var cluster_names = {
    1: "Map",
    3: "ADF+AI-FinSi+AIAfrica+AIFS-Bridge+AISRF+AiFin+Benchmarks+ECONLP+FAIF+FEAP-AI4Fin+FNP+Fin-RecSys+FinNLP+FinWeb+GLIndA+KDF+MAL+ML-ESG+MLF+MLInv+Muffin+NLP-NA-Finance+RAIFS+RobustFin+SynFin+WFS+WomenAIFin+XAIFIN",
    11: "FinIR",
    7: "FinRec",
    2: "DMF+FinLLM",
    8: "DSMM",
...
function configSigmaElements(config) {
    ...
    var a = [],
        ...
        {
            color_to_cluster[b] = x
            a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div> ' + (cluster_names[x++]) + ' (' + sigInst.clusters[b].length + ' members)</a></div>');
        }
    //a.sort();
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        sigInst.draw(2, 2, 2, 2);
        $GP.info_name.html("<b>" + cluster_names[color_to_cluster[a]] + "</b>");
        $GP.info_data.hide();
